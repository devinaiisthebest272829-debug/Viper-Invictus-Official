...
function printBanner() {
  if (NO_COLOR) {
    console.log("Viper Invictus v1.4");
    return;
  }
  console.log(`
...
${C.magenta}${C.bold}   ╚████╔╝ ██║██║     ███████╗██║  ██║${C.reset}
${C.magenta}${C.bold}    ╚═══╝  ╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝${C.reset}
${C.dim}  Invictus CLI v1.4  —  The language that outlasts C++${C.reset}
`);
}
...
async function startREPL(opts: { trusted?: boolean; verbose?: boolean } = {}) {
  ...
  console.log(`${bold("Viper REPL v1.4")} — Type ${cyan(".help")} for commands, ${cyan(".exit")} to quit\n`);
...
async function main() {
  ...
  if (argv.includes("--version") || cmd === "version") {
    console.log("Viper Invictus CLI v1.5.0");
    process.exit(0);
  }
