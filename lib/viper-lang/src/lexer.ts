...
export function lex(src: string): { tokens: Token[]; errors: Array<{ message: string; line: number }> } {
  ...
  while (pos < src.length) {
    ...
    switch (ch) {
      ...
      case ".": tokens.push(match(".") ? (match(".") ? tok(TokenType.Spread, "...", startLine, startCol) : tok(TokenType.Spread, "..", startLine, startCol)) : tok(TokenType.Dot, ".", startLine, startCol)); break;
